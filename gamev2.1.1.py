...
class Level:
        ...
        for row, line in enumerate(lines):
            for col, char in enumerate(line):
                ...
                if char == "#":
                    ...
                    self.coins.append(Coin(x, y, assets["coin"], phase=(col + row) * 0.7))
                elif char == "g":
    ...
    def draw(self, screen: pygame.Surface, camera_x: int, frame_count: int, effects: list[object]) -> None:
        ...
        for coin in self.coins:
            if self.visible(coin.rect, camera_x):
                coin.draw(screen, camera_x, frame_count)
...
class MarioGame:
    ...
    def handle_collisions(self) -> None:
        ...
        self.level.coins = remaining_coins
        ...
        for proj in self.level.projectiles:
            ...
            for enemy in list(self.level.enemies):
                if proj.rect.colliderect(enemy.rect):
                    ...
                    if proj.p_type == "fire":
                        self.level.enemies.remove(enemy)
                        self.effects.append(DefeatEffect(enemy.rect.x - enemy.HITBOX_X, enemy.rect.y - enemy.HITBOX_Y,
                                                         self.assets["enemy_flat"]))
                        self.score += 100
