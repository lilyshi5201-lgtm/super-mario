...
class MarioGame:
    ...
    def add_coins(self, amount: int, score_per_coin: int = 50) -> None:
        ...
        gained_lives = (self.coins // 20) - (previous_coins // 20)
        if gained_lives > 0:
    ...
    def handle_punch_hits(self, solid_rects: list[pygame.Rect]) -> None:
        punch_rect = self.player.punch_rect()
        if punch_rect is None:
            return
        for enemy in list(self.level.enemies):
            if not punch_rect.colliderect(enemy.rect):
                continue
            if enemy.apply_punch_damage(self.player.facing, solid_rects):
    ...
    def update(self) -> None:
        ...
        if self.player.punch_timer > 0:
            ...
            solid_rects = self.level.solid_rects()
            self.handle_punch_hits(solid_rects)

        if self.player.ceiling_hit_rect is not None:
            self.level.hit_tiles_from_collision(self.player.ceiling_hit_rect)
    ...
    def draw_hud(self) -> None:
        ...
        if self.player.power_timer > 0:
            power_str += f" ({self.player.power_timer // FPS}s)"
